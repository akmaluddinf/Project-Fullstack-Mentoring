...
class ServiceRequestModel(db.Model):
    ...
    def __init__(self, requester_name, requester_payroll, status):
    ...
    def serialize(self):
        return{
            ...
            'comments' : [{'comment_id' : item.id, 'comment' : item.comment, 'created_date' : item.created_date, 'service_request_id' : item.service_request_id} for item in self.comments]
        }
...
class ServiceItems(db.Model):
    ...
    def __init__(self, service_id, service_type, description, quantity, unit_price):
        self.service_id = service_id
        self.service_type = service_type
...
def service_request():
    if request.method == 'POST':
        if request.is_json:
            ...
            # new_comment = Comments(comment=data['comment'])
            # new_service_request.comments.append(new_comment)
            ...
            # new_service_request.service_items.append(new_service_item)

            for item in data["service_items"]:
                new_serivce_item =  ServiceItems(
                    service_id= item['service_id'], 
                    service_type=item['service_type'],
                    description = item['description'],
                    quantity = item['quantity'],
                    unit_price = item['unit_price'])
                new_service_request.service_items.append(new_serivce_item)
        ...
        #return {"count": len(results), "results": results, "message": "success"}
...
def update_service(id):
    service = ServiceModel.query.get_or_404(id)
...
def error405(error):
    return "Metode Tidak Diperbolehkan" 
